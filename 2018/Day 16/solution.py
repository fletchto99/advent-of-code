valid_instructions = [
    {
        'name': 'addr',
        'operation': lambda registers, params:
    ...
    },
    {
        'name': 'gtir',
        'operation': lambda registers, params:
            1 if params[0] > registers[params[1]] else 0,
        'potential_opcodes': set()
    },
    {
        'name': 'gtri',
        'operation': lambda registers, params:
            1 if registers[params[0]] > params[1] else 0,
        'potential_opcodes': set(),
    },
    {
        'name': 'gtrr',
...
for test in samples:
    for instruction in valid_instructions:
        result = test['input'][:]
        result[test['params'][2]] = instruction['operation'](
            result, test['params'])
...
# Oof there's gotta be a better way to do this
# using set differences or something
        ...
        if len(instruction['potential_opcodes']) == 1:
            ...
            for instruction2 in valid_instructions:
                if instruction['opcode'] in instruction2['potential_opcodes']:
                    instruction2['potential_opcodes'].remove(
                        instruction['opcode'])
...
for line in program:
    instruction = list(filter(
        lambda instr: instr['opcode'] == line['opcode'],
        valid_instructions
    ))[0]
