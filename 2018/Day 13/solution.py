...
while True:
    ...
    if not cart['removed']:
      if cart['direction'] == "^":
        ...
        if track_piece == "/":
          ...
          cart['intersection'] += 1
      for cart2 in carts:
        if cart2 != cart and cart2['x'] == cart['x'] and cart2['y'] == cart['y'] and cart['removed'] == False and cart2['removed'] == False:
          if not collide:
            print("Collision at: " + str(cart['x']) + "," + str(cart['y']))
            collide = True
          cart['removed'] = True
          cart2['removed'] = True
          break
  ...
  if len(filtered) == 1:
    print("Last cart is: " + str(filtered[0]['x']) + "," + str(filtered[0]['y']))
    break
