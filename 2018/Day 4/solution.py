...
for line in open('input.txt'):
    ...
    if "wakes" in splt[2]:
        ...
        to_add = {'guard': guard, 'sleeping_time': dict(
            (el, 0) for el in range(60))}
...
res = int(max(match['sleeping_time'].items(),
              key=operator.itemgetter(1))[0]) * most
print(res)

for guard in guards:
    guard['most'] = max(guard['sleeping_time'].items(),
