...
for i in range(iterations):
  for j in range(2, len(current) - 2):
    ...
    if not found:
      nextGen.append('.')
  nextGen = nextGen + ['.','.']
  current = ['.','.'] + nextGen.copy() + ['.','.']
  nextGen = ['.','.']
  sums.append(sum([idx-((i+1)*2+3) for idx,plant in enumerate(current) if plant == "#"]))

print(sums[19])
print(str(((50000000000 * (sums[499]-sums[498])) - ((sums[499]-sums[498]) * 500)) + sums[499]))
